...
internal class PoTokenWebView4 private constructor(
    ...
    fun downloadAndRunBotguard() {
        ...
        // YouTube now ties the BotGuard challenge to homepage ytcfg/EVENT_ID.
        // The old /att/get-only challenge can mint a token that looks valid but GVS later rejects.
        val challenge = getChallengeFromHomepage() ?: getLegacyChallengeData() ?: return
        val parsedChallengeData = challenge.first
        val ytcfg = challenge.second

        runOnMainThread {
            webView.evaluateJavascriptLegacy(
                """try {
                    ${if (ytcfg != null) "yt = { config_: $ytcfg };" else ""}
    ...
    /** Fetch a self-consistent homepage ytcfg + ytAtN challenge pair. */
    private fun getChallengeFromHomepage(): Pair<String, String?>? {
        ...
        val ytcfgMatcher = Pattern
            .compile("""ytcfg\.set\((\{.+?\})\);""", Pattern.DOTALL)
            .matcher(pageHtml)
        val ytcfg = if (ytcfgMatcher.find()) ytcfgMatcher.group(1) else null
        if (ytcfg == null) {
            Log.w(TAG, "homepage-challenge: no ytcfg found (EVENT_ID missing)")
        }

        val attMatcher = Pattern
            .compile("""window\.ytAtN\(\s*(\{[\s\S]*?\})\s*\)""")
            .matcher(pageHtml)
        if (!attMatcher.find()) {
        ...
        Log.d(TAG, "Using challenge from the homepage (SABR/GVS compatible)")
        return Pair(parseDescrambledChallengeData(rawChallengeData), ytcfg)
    }

    /** Fallback used only if the homepage challenge cannot be obtained. */
    private fun getLegacyChallengeData(): Pair<String, String?>? {
        ...
        Log.w(TAG, "Using legacy /att/get challenge fallback")
    ...
    fun onRunBotguardResult(botguardResponse: String) {
        ...
        Log.d(TAG, "GenerateIT response received, len=${responseBody.length}")
    ...
    override fun generatePoToken(identifier: String): String {
        Log.d(TAG, "generatePoToken() called, identifierLen=${identifier.length}")
    ...
    fun onObtainPoTokenError(identifier: String, error: String) {
        val msg = "onObtainPoTokenError: identifierLen=${identifier.length} error=$error"
        Log.e(TAG, msg)
    ...
    fun onObtainPoTokenResult(identifier: String, poTokenU8: String) {
        Log.d(TAG, "Generated poToken bytes: identifierLen=${identifier.length} byteStringLen=${poTokenU8.length}")
        val poToken = u8ToBase64(poTokenU8)

        Log.d(TAG, "Generated poToken: identifierLen=${identifier.length} poTokenLen=${poToken.length}")
    ...
    private fun makeBotguardServiceRequest(
        ...
        val response = if (data == null) {
            OkHttpManager.instance().doGetRequest(url, requestHeaders)
        } else {
            OkHttpManager.instance().doPostRequest(url, requestHeaders, data, null)
        }
