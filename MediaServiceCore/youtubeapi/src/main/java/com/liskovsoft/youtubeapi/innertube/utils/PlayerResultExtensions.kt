...
internal fun PlayerResult.getMergeCaptionTracks(): List<CaptionTrack>? {
    ...
    if (translationLanguages != null || originTrack.isTranslatable == true) {
        // Keep a deterministic Polish fallback even when the response doesn't expose a
        // translationLanguages array (common for videos with several authored captions).
        resultTracks.addTranslatedTrack(originTrack, POLISH_LANGUAGE_CODE, POLISH_LANGUAGE_NAME,
            "mobile.$POLISH_LANGUAGE_CODE")
    }
    if (translationLanguages == null)
        return resultTracks

    for (language in translationLanguages) {
        resultTracks.addTranslatedTrack(originTrack, language.languageCode,
            language.languageName?.getText())
    }

    return resultTracks
}

private fun MutableList<CaptionTrack>.addTranslatedTrack(originTrack: CaptionTrack,
                                                          languageCode: String?,
                                                          languageName: String?,
                                                          idSuffix: String? = null) {
    if (languageCode.isNullOrBlank()) return
    // Don't try to translate the same lang or you'll get a mess.
    val baseUrl = if (originTrack.languageCode == languageCode) originTrack.baseUrl
        else "${originTrack.baseUrl}&tlang=$languageCode"
    add(CaptionTrack(
        baseUrl,
        originTrack.isTranslatable,
        languageCode,
        translatedVssId(originTrack.vssId, languageCode, idSuffix),
        TextItem(null, YouTubeHelper.exoNameFix(languageName), null, null),
        TYPE_ASR // add autogenerated marker
    ))
}

private fun translatedVssId(originVssId: String?, languageCode: String,
                            idSuffix: String?): String {
    // ExoPlayer uses vssId as the format id. Reusing the origin id for every translation
    // makes different languages collide and may select captions other than the tapped entry.
    return "${originVssId ?: "caption"}.tlang.${idSuffix ?: languageCode}"
}
/**
...
private const val POLISH_LANGUAGE_NAME = "Polski"
