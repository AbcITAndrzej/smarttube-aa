...
set "CRASH_LOGS_DIR=%RELEASES_DIR%\crash-logs"
...
if /I "%~1"=="--shorts-crash" goto SHORTS_CRASH_CAPTURE
...
echo 20 - SHORTS CRASH: zbierz logi awarii bez recznego ADB
echo  0 - Wyjscie
echo.
set "OPT="
...
if "%OPT%"=="20" goto SHORTS_CRASH_CAPTURE
if "%OPT%"=="0" exit /b 0
goto MENU
...
echo.
echo ============================================================
echo   SHORTS CRASH - AUTOMATYCZNE ZBIERANIE LOGOW
echo ============================================================
echo Telefon: !SELECTED_MODEL! [!SELECTED_DEVICE!]
echo.
echo  1 - Music      ^(%SMARTUBE_MOBILE_PACKAGE%^)
echo  2 - Video EXP  ^(%SMARTUBE_VIDEO_PACKAGE%^)
echo.
set "CRASH_APP_PICK="
set /p "CRASH_APP_PICK=Wybierz aplikacje [1/2, Enter = 1]: "
if not defined CRASH_APP_PICK set "CRASH_APP_PICK=1"
if "!CRASH_APP_PICK!"=="1" (
  set "CRASH_PACKAGE=%SMARTUBE_MOBILE_PACKAGE%"
  set "CRASH_APP_NAME=Music"
) else if "!CRASH_APP_PICK!"=="2" (
  set "CRASH_PACKAGE=%SMARTUBE_VIDEO_PACKAGE%"
  set "CRASH_APP_NAME=Video-EXP"
) else (
  echo [BLAD] Niepoprawny wybor aplikacji.
  pause
  goto MENU
)

"!ADB_EXE!" -s "!SELECTED_DEVICE!" shell pm path "!CRASH_PACKAGE!" >nul 2>&1
if errorlevel 1 (
  echo [BLAD] Pakiet !CRASH_PACKAGE! nie jest zainstalowany na wybranym telefonie.
  pause
  goto MENU
)

for /f "usebackq delims=" %%T in (`powershell -NoProfile -Command "Get-Date -Format yyyyMMdd-HHmmss"`) do set "STAMP=%%T"
set "CRASH_SESSION_DIR=%CRASH_LOGS_DIR%\Shorts-!CRASH_APP_NAME!-!STAMP!"
if not exist "!CRASH_SESSION_DIR!" mkdir "!CRASH_SESSION_DIR!"
if errorlevel 1 (
  echo [BLAD] Nie moge utworzyc katalogu:
  echo        !CRASH_SESSION_DIR!
  pause
  goto MENU
)

echo.
echo [INFO] Czyszcze stary logcat, aby raport dotyczyl tylko tego testu...
"!ADB_EXE!" -s "!SELECTED_DEVICE!" logcat -c >nul 2>&1

(
  echo SmartTube AA - Shorts crash capture
  echo captured=%DATE% %TIME%
  echo device=!SELECTED_MODEL!
  echo serial=!SELECTED_DEVICE!
  echo abi=!SELECTED_ABI!
  echo package=!CRASH_PACKAGE!
  echo app=!CRASH_APP_NAME!
) > "!CRASH_SESSION_DIR!\README.txt"

"!ADB_EXE!" -s "!SELECTED_DEVICE!" shell getprop > "!CRASH_SESSION_DIR!\device-getprop.txt" 2>&1
"!ADB_EXE!" -s "!SELECTED_DEVICE!" shell dumpsys package "!CRASH_PACKAGE!" > "!CRASH_SESSION_DIR!\package.txt" 2>&1

echo [INFO] Uruchamiam !CRASH_APP_NAME!...
"!ADB_EXE!" -s "!SELECTED_DEVICE!" shell am force-stop "!CRASH_PACKAGE!" >nul 2>&1
"!ADB_EXE!" -s "!SELECTED_DEVICE!" shell monkey -p "!CRASH_PACKAGE!" -c android.intent.category.LAUNCHER 1 >nul 2>&1

echo.
echo ============================================================
echo TERAZ NA TELEFONIE:
echo   1. Wejdz w Shorts.
echo   2. Odtworz rolke.
echo   3. Niczego nie dotykaj az wystapi blad / crash.
echo   4. PO AWARII wroc do tego okna i nacisnij dowolny klawisz.
echo.
echo Nie zamykaj helpera. Nie musisz wpisywac zadnych komend ADB.
echo ============================================================
echo.
pause

echo.
echo [INFO] Zbieram logcat...
"!ADB_EXE!" -s "!SELECTED_DEVICE!" logcat -d -v threadtime > "!CRASH_SESSION_DIR!\logcat-full.txt" 2>&1

echo [INFO] Wycinam najwazniejsze linie crasha...
findstr /I /C:"FATAL EXCEPTION" /C:"AndroidRuntime" /C:"Process: !CRASH_PACKAGE!" /C:"ANR in !CRASH_PACKAGE!" /C:"OutOfMemoryError" /C:"ExoPlayer" /C:"MediaCodec" /C:"SIGABRT" /C:"SIGSEGV" /C:"Fatal signal" "!CRASH_SESSION_DIR!\logcat-full.txt" > "!CRASH_SESSION_DIR!\logcat-crash-summary.txt" 2>nul

echo [INFO] Zbieram Android exit-info...
"!ADB_EXE!" -s "!SELECTED_DEVICE!" shell dumpsys activity exit-info "!CRASH_PACKAGE!" > "!CRASH_SESSION_DIR!\activity-exit-info.txt" 2>&1

echo [INFO] Zbieram stan aktywnosci/procesow...
"!ADB_EXE!" -s "!SELECTED_DEVICE!" shell dumpsys activity processes > "!CRASH_SESSION_DIR!\activity-processes.txt" 2>&1
"!ADB_EXE!" -s "!SELECTED_DEVICE!" shell dumpsys meminfo "!CRASH_PACKAGE!" > "!CRASH_SESSION_DIR!\meminfo.txt" 2>&1

echo [INFO] Probuje pobrac wbudowane logi diagnostyczne SmartTube...
if not exist "!CRASH_SESSION_DIR!\app-diagnostics" mkdir "!CRASH_SESSION_DIR!\app-diagnostics"
"!ADB_EXE!" -s "!SELECTED_DEVICE!" pull "/sdcard/Android/data/!CRASH_PACKAGE!/files/diagnostic-logs" "!CRASH_SESSION_DIR!\app-diagnostics" > "!CRASH_SESSION_DIR!\diagnostic-pull.txt" 2>&1
if errorlevel 1 (
  echo [INFO] Android nie pozwolil pobrac katalogu diagnostic-logs przez ADB lub katalog jeszcze nie istnieje.
  echo        Sam logcat + exit-info zostaly zapisane poprawnie.
) else (
  echo [OK] Pobrano rowniez wbudowane logi diagnostyczne aplikacji.
)

(
  echo.
  echo ===== FILES =====
  dir /b "!CRASH_SESSION_DIR!"
) >> "!CRASH_SESSION_DIR!\README.txt"

echo.
echo ============================================================
echo [OK] RAPORT SHORTS CRASH GOTOWY
echo ============================================================
echo Katalog:
echo !CRASH_SESSION_DIR!
echo.
echo Najwazniejsze pliki:
echo   logcat-full.txt
echo   logcat-crash-summary.txt
echo   activity-exit-info.txt
echo   package.txt
echo   meminfo.txt
echo   app-diagnostics\  ^(jesli Android pozwolil pobrac^)
echo.
echo Otwieram katalog. Mozesz spakowac caly folder i mi go wyslac.
echo ============================================================
start "" "!CRASH_SESSION_DIR!"
echo.
pause
goto MENU
