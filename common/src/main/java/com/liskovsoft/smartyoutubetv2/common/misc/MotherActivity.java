...
public class MotherActivity extends FragmentActivity {
    ...
    private boolean mIsMobileTouchNavigationEnabled;
    private MobileTouchController mMobileTouchController;
    private MobileUiCompatibility mMobileUiCompatibility;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        mIsMobileTouchNavigationEnabled = getResources().getBoolean(R.bool.mobile_touch_navigation_enabled);
        MobileDiagnostics.lifecycle(this, "onCreate", "mobile=" + mIsMobileTouchNavigationEnabled);
        // Orientation is supplied by the stmobile manifest. Do not force TV landscape here.
        ...
        if (!mIsFullscreenModeEnabled && !mIsMobileTouchNavigationEnabled) { // P12: do not apply FitSystemWindows on phones
        ...
        if (mIsMobileTouchNavigationEnabled) {
            // The player already contains native double-tap/seek handling. A global
            // DPAD translator there would consume the same touch sequence twice.
            if (!MobileUiCompatibility.isPlaybackActivity(this)) {
                mMobileTouchController = new MobileTouchController(this);
            }
            if (getResources().getBoolean(R.bool.mobile_settings_ui_adaptation_enabled)) {
                mMobileUiCompatibility = new MobileUiCompatibility(this);
                mMobileUiCompatibility.attach();
            }
        } else {
            initEdgeSlide();
        }
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        if (mMobileTouchController != null && mMobileTouchController.onTouchEvent(event)) {
            return true;
        }
    ...
    private DisplayMetrics getDisplayMetrics(Context context) {
        ...
        if (sCachedDisplayMetrics == null) {
            ...
            if (mIsMobileTouchNavigationEnabled) {
                int mobileScalePercent = getResources().getInteger(R.integer.mobile_ui_scale_percent);
                mobileScalePercent = Math.max(75, Math.min(125, mobileScalePercent));
                uiScale *= mobileScalePercent / 100f;
            }
    ...
    private void applyFullscreenModeIfNeeded() {
        boolean forceMobileFullscreen = mIsMobileTouchNavigationEnabled; // P12: mobile flavor always uses immersive fullscreen
        if (mIsFullscreenModeEnabled || forceMobileFullscreen) {
    ...
    protected void onDestroy() {
        MobileDiagnostics.lifecycle(this, "onDestroy", "finishing=" + isFinishing());
        if (mMobileUiCompatibility != null) {
            mMobileUiCompatibility.release();
            mMobileUiCompatibility = null;
        }
        if (mMobileTouchController != null) {
            mMobileTouchController.cancel();
            mMobileTouchController = null;
        }
        super.onDestroy();
    }

    protected final boolean isMobileTouchNavigationEnabled() {
        return mIsMobileTouchNavigationEnabled;
    }
