...
public class GridFragmentHelper {
    ...
    private static int sLastDisplayWidthPx = -1;
    ...
    private static float getMaxColsNumFloat(Context context, int cardWidthResId, float cardScale) {
        int currentDisplayWidthPx = context.getResources().getDisplayMetrics().widthPixels;
        if (sLastDisplayWidthPx != currentDisplayWidthPx) {
            invalidate();
            sLastDisplayWidthPx = currentDisplayWidthPx;
        }
    ...
    private static float getMaxColsNumFloatInt(Context context, int cardWidthResId, float cardScale) {
        ...
        // P12.2 mobile: use the real current width. The TV code used the
        // larger screen dimension even in portrait, creating too many
        // columns and a grid that could be dragged outside the phone.
