...
public class NavigateTitleView extends TitleView implements OnDataChange, AccountChangeListener {
    private SearchOrbView mMenuView;
    ...
    public void updateComponentsVisibility(int flags) {
        ...
        if (mMenuView != null) {
            mMenuView.setVisibility(mSearchVisibility);
        }
    ...
    private void setupButtons() {
        ...
        mMenuView = findViewById(R.id.menu_orb);
        if (mMenuView != null) {
            mMenuView.setOnOrbClickedListener(v -> {
                BrowseActivity activity = findBrowseActivity();
                if (activity != null) {
                    activity.toggleBrowseSidebar();
                }
            });
            String menuTooltip = getContext().getString(R.string.toggle_sidebar);
            mMenuView.setContentDescription(menuTooltip);
            TooltipCompatHandler.setTooltipText(mMenuView, menuTooltip);
        }
    ...
    private BrowseActivity findBrowseActivity() {
        Context context = getContext();

        while (context instanceof ContextWrapper) {
            if (context instanceof BrowseActivity) {
                return (BrowseActivity) context;
            }
        ...
        return context instanceof BrowseActivity ? (BrowseActivity) context : null;
    }
